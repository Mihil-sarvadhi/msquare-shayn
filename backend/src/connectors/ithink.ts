...
const BASE = process.env.ITHINK_BASE_URL as string;
const AUTH = {
...
export interface IthinkOrderResponse {
  status: string;
  data?: Record<string, IthinkOrder>;
}
...
export async function getRemittanceDetails(date: string): Promise<unknown> {
