...
type CA = Optional<
  Attrs,
  'id' | 'last_sync_at' | 'status' | 'error_message' | 'records_synced' | 'updated_at'
>;

export class ConnectorHealth extends Model<Attrs, CA> implements Attrs {
  declare id?: number;
  declare connector_name: string;
...
ConnectorHealth.init(
  {
    ...
    status: { type: DataTypes.TEXT, defaultValue: 'unknown' },
    error_message: DataTypes.TEXT,
    records_synced: { type: DataTypes.INTEGER, defaultValue: 0 },
