...
import type {
  KpiResult, RevenueTrendRow, MetaFunnelRow, CampaignRow, TopProductRow,
  LogisticsRow, AbandonedCartsRow, ReviewsSummaryRow, ReviewsTrendRow,
  TopRatedProductRow, RecentReviewRow, AllReviewsResult,
...
export async function getKpis(since: string, until: string): Promise<KpiResult> {
  const [shopifyKPIs, metaKPIs, ithinkKPIs] = await Promise.all([
    sequelize.query<{
      total_revenue: string; total_orders: string; aov: string;
      unique_customers: string; cod_orders: string; prepaid_orders: string;
    }>(
      `SELECT
    ...
    sequelize.query<{
      total_spend: string; total_impressions: string; total_clicks: string;
      total_purchases: string; total_purchase_value: string; roas: string;
    }>(
      `SELECT
    ...
    sequelize.query<{
      total_shipments: string; delivered: string; rto: string; ofd: string; ndr: string;
    }>(
      `SELECT
  ...
  return {
    revenue: parseFloat(s.total_revenue), orders: parseInt(s.total_orders, 10),
    aov: parseFloat(s.aov), customers: parseInt(s.unique_customers, 10),
    codOrders: parseInt(s.cod_orders, 10), prepaidOrders: parseInt(s.prepaid_orders, 10),
    adSpend: parseFloat(m.total_spend), impressions: parseInt(m.total_impressions, 10),
    clicks: parseInt(m.total_clicks, 10), purchases: parseInt(m.total_purchases, 10),
    purchaseValue: parseFloat(m.total_purchase_value), roas: parseFloat(m.roas),
    totalShipments: parseInt(i.total_shipments, 10), delivered: parseInt(i.delivered, 10),
    rto: parseInt(i.rto, 10), ofd: parseInt(i.ofd, 10), ndr: parseInt(i.ndr, 10),
...
export async function getReviewsSummary(): Promise<ReviewsSummaryRow> {
...
export async function getTopRatedProducts(): Promise<TopRatedProductRow[]> {
...
export async function getRecentReviews(): Promise<RecentReviewRow[]> {
  return sequelize.query<RecentReviewRow>(
    `SELECT r.review_id, r.rating, r.title, r.body, r.reviewer_name,
     ...
     FROM judgeme_reviews r WHERE r.published = TRUE ORDER BY r.created_at DESC LIMIT 10`,
    { type: QueryTypes.SELECT }
  );
}

export async function getAllReviews(
  page: number, limit: number, rating: number, search: string
  ...
  if (search) {
    conditions.push('(r.title ILIKE :search OR r.body ILIKE :search OR r.reviewer_name ILIKE :search)');
