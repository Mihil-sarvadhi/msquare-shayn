...
import { refreshTokenJob as refreshUnicommerceTokenJob } from '@modules/unicommerce/unicommerce.token';
...
export function startScheduler(): void {
  ...
  // Every 15 min — Unicommerce marketplace orders (Flipkart, Amazon, Myntra, Eternz)
  cron.schedule(
    '*/15 * * * *',
    async () => {
      logger.info('[Cron] Running Unicommerce sync...');
      await syncUnicommerce();
    },
    TZ,
  );

  // Every 7 hours — Unicommerce token refresh (token TTL is ~8h)
  cron.schedule(
    '0 */7 * * *',
    async () => {
      logger.info('[Cron] Refreshing Unicommerce token...');
      await refreshUnicommerceTokenJob();
    },
    TZ,
  );
